...
library(rmsfuns)
load_pkg(c("tidyverse","SentimentAnalysis","tbl2xts","ggthemes","ggplot2", "furrr", "viridisLite", "viridis", "lubridate"))
```
...
political_comps <-  political_naive %>% 
  select(year_month) %>%
  mutate(naive = political_naive$composite) %>% 
  mutate(refined= political_refine$composite) %>% 
...
j+ geom_rect(data=data.frame(xmin=decimal_date(as.Date(c("2007-11"), format = "%Y-%m")),
                            xmax=decimal_date(as.Date(c("2009-12"), format = "%Y-%m")),
                            ymin=-Inf,
                            ymax=Inf
                            ))
...
```{r}
test <-political_refine %>% 
  select(year_month, composite) 

j <- test %>%
  ggplot(aes(x=year_month, y=composite, group = 1), inherit.aes = FALSE) +
  geom_line()+
    theme(legend.position="none") +
    ggtitle("Naive- and refined political uncertainty index") +
    theme_base()+
