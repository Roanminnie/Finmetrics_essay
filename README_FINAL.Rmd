...
library(rmsfuns)
load_pkg(c("tidyverse","SentimentAnalysis","tbl2xts","ggthemes","ggplot2", "furrr", "viridisLite", "viridis", "lubridate", "zoo", "strucchange", "GGally", "pastecs", "data.table"))
```
...
political_naive %>%
  ...
  ggplot( aes(x=year_month, y=value, group=type, color=highlight, size=highlight)) +
    ...
    scale_x_discrete(breaks = c("2004-01", "2004-04","2004-07","2004-12","2005-03","2005-06","2005-09","2005-12", "2006-04","2006-07","2006-11","2007-02","2007-05","2007-08","2007-11","2008-03","2008-07","2008-10","2009-03", "2009-06","2009-10" ,"2010-01","2010-04","2010-07" , "2010-10" ,"2011-01", "2011-04","2011-07", "2011-10", "2012-02" , "2012-05", "2012-08" ,"2012-11" ,"2013-04", "2013-08","2013-12", "2014-04" , "2014-08", "2015-07" , "2015-11" ,"2016-02" , "2016-05" ,"2016-09" , "2016-12", "2017-03" ,"2017-06" , "2017-09"))+
...
fiscal_naive %>%
  ...
  ggplot( aes(x=year_month, y=value, group=type, color=highlight, size=highlight)) +
    ...
    scale_size_manual(values=c(1.5,0.2)) +
    scale_x_discrete(breaks = c("2004-01","2004-04","2004-07", "2004-10", "2005-01","2005-04" , "2005-07", "2005-10"
 , "2006-01","2006-04" ,"2006-07" , "2006-10" ,"2007-01" ,"2007-04", "2007-07", "2007-10", "2008-01", "2008-04" ,"2008-07","2008-10" ,"2009-01" , "2009-04", "2009-07", "2009-10" , "2010-01", "2010-04" ,"2010-07" , "2010-10" ,"2011-01" , "2011-04","2011-07","2011-10" , "2012-01","2012-04", "2012-07" ,"2012-10","2013-01" , "2013-04" , "2013-07" ,"2013-10" , "2014-01","2014-04" , "2014-07" , "2014-10","2015-06" ,"2015-09","2015-12" , "2016-03" , "2016-06", "2016-09", "2016-12", "2017-03","2017-06" ,"2017-09"))+
    theme(legend.position="none") +
...
ggsave("bin/fiscal_comp_naive2.jpeg")
...
monetary_naive %>%
  ...
  ggplot( aes(x=year_month, y=value, group=type, color=highlight, size=highlight)) +
    ...
    scale_size_manual(values=c(1.5,0.2)) +
    theme(legend.position="none") +
    scale_x_discrete(breaks = c("2004-01" , "2004-04","2004-07" ,"2004-10" ,"2005-01", "2005-04", "2005-07" ,"2005-10", "2006-01" , "2006-04" , "2006-07" ,
...
rectangle1 <- data.frame(xmin = "2015-08",
                        xmax = "2017-09",
                        ymin = -Inf, ymax = Inf)
...
pol_comps <- political_comps %>%
  ...
  scale_x_discrete(breaks = c("2007-11", "2010-02","2015-08", "2017-09"))
...
fiscal_comps %>%
  ggplot(aes(x=year_month, y=value, group=index, color = index)) +
  geom_rect(data = rectangle1, aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax), 
              fill ="#eb8055ff" , alpha = 0.2,inherit.aes = FALSE )+
  geom_rect(data = rectangle2, aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax), 
              fill ="#eb8055ff" , alpha = 0.2,inherit.aes = FALSE )+
  geom_line(aes(size = index))+
  scale_size_manual(values=c(0.2, 1.5)) +
  scale_color_manual(values = c("grey","#eb8055ff"))+
  theme(legend.position="none") +
...
financial_comps %>%
  ggplot(aes(x=year_month, y=value, group=index, color = index)) +
  geom_rect(data = rectangle1, aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax), 
              fill ="#b0dd31ff", alpha = 0.2,inherit.aes = FALSE )+
  geom_rect(data = rectangle2, aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax), 
              fill ="#b0dd31ff" , alpha = 0.2,inherit.aes = FALSE )+
  geom_line(aes(size = index))+
  scale_size_manual(values=c(0.2, 1.5)) +
  scale_color_manual(values = c("grey","#b0dd31ff"))+
  theme(legend.position="none") +
...
first_cohort <- rawdata %>% 
  filter(year_month<="2010-02" & year_month>="2007-11") %>% 
  group_by(PubDate) %>%
  mutate(us = mean(refine)) %>% ungroup() 


second_cohort <- rawdata %>% 
  filter(year_month<="2017-07" & year_month>="2015-08") %>% 
  group_by(PubDate) %>%
  mutate(us = mean(refine)) %>% ungroup() 
write_rds(first_cohort, "data/first.rds")
write_rds(second_cohort, "data/second.rds")

library("plyr")

temp<- second %>% filter(year_month=="2017-07") %>% select(year_month, PubDate, us) %>% 
  unique() %>% arrange(us, decreasing=TRUE) %>% 
  head(., n = 5) %>% arrange(PubDate)

temp<- first %>% filter(year_month=="2010-02") %>% select(year_month, PubDate, us) %>% 
  unique() %>% arrange(us, decreasing=TRUE) %>% 
  head(., n = 5) %>% arrange(PubDate)

test <- financial_comps %>% 
  filter(index == "refined") %>% 
  filter(year_month<="2010-02" & year_month>="2007-11")
```

##Breakpoint analysis
```{r}
ts_pol <- pol_refine_pe %>% 
  select(year_month, composite)
ts_pol <- ts(ts_pol$composite, start=c(1900,1), end=c(2039, 1), frequency=1)

bp_ts <- breakpoints(ts_pol ~ 1)
summary(bp_ts)
plot(bp_ts)
plot(ts_pol)
lines(bp_ts)

breakpoint_pol <- pol_refine_pe %>% 
  select(composite, year_month) %>% 
  mutate(bp = seq(1900, 2038))

ts_fis <- fis_refine_pe %>% 
  select(year_month, composite)
ts_fis <- ts(ts_fis$composite, start=c(1900,1), end=c(2056, 1), frequency=1)

bp_ts <- breakpoints(ts_fis ~ 1)
summary(bp_ts)
plot(bp_ts)
plot(ts_fis)
lines(bp_ts)

breakpoint_fis <- fis_refine_pe %>% 
  select(composite, year_month) %>% 
  mutate(bp = seq(1900, 2056))

ts_mon <- mon_refine_pe %>% 
  select(year_month, composite)
ts_mon <- ts(ts_mon$composite, start=c(1900,1), end=c(2011, 1), frequency=1)

bp_ts <- breakpoints(ts_mon ~ 1)
summary(bp_ts)
plot(bp_ts)
plot(ts_mon)
lines(bp_ts)

breakpoint_mon <- mon_refine_pe %>% 
  select(composite, year_month) %>% 
  mutate(bp = seq(1900, 2011))


ts_fin <- fin_refine_pe %>% 
  select(year_month, composite)
ts_fin <- ts(ts_fin$composite, start=c(1900,1), end=c(2045, 1), frequency=1)

bp_ts <- breakpoints(ts_fin ~ 1)
summary(bp_ts)
plot(bp_ts)
plot(ts_fin)
lines(bp_ts)

breakpoint_fin <- fin_refine_pe %>% 
  select(composite, year_month) %>% 
  mutate(bp = seq(1900, 2045))
...
summ <- pol_refine_pe %>% 
  select(year_month) %>% 
  mutate(political = pol_refine_pe$composite) 

summ1 <-mon_refine_pe %>% 
  select(year_month) %>% 
  mutate(monetary = mon_refine_pe$composite) 

setDT(summ)
setDT(summ1)

comb <- summ1[summ, on = c('year_month')]

summ2 <- fis_refine_pe %>% 
  select(year_month) %>% 
  mutate(fiscal = fis_refine_pe$composite) 
  
setDT(summ2)
setDT(comb)

comb1 <- comb[summ2, on = c('year_month')]

summ3 <- fin_refine_pe %>% 
  select(year_month) %>% 
  mutate(financial = fin_refine_pe$composite) 
  
setDT(summ3)
setDT(comb1)

comb2 <- comb1[summ3, on = c('year_month')]
final_comb <- na.omit(comb2) %>% select(-year_month)
...
#Constructuting summary table by hand
stat_pol <- stat.desc(pol_refine_pe$composite)
stat_mon <- stat.desc(mon_refine_pe$composite)
stat_fis <- stat.desc(fis_refine_pe$composite)
stat_fin <- stat.desc(fin_refine_pe$composite)

summ <- data.frame(row.names = c("Min", "Max", "Std.dev"))
summ <- add_column(summ, monetary = c(stat_mon[4], stat_mon[5], stat_mon[13]))
summ <- add_column(summ, political = c(stat_pol[4], stat_pol[5], stat_pol[13]))
summ <- add_column(summ, fiscal = c(stat_fis[4], stat_fis[5], stat_fis[13]))
summ <- add_column(summ, financial = c(stat_fin[4], stat_fin[5], stat_fin[13]))
...
```{r}
library(readxl)
comparative <- read_excel("data/South_African_Uncertainty_Indices_Q1_2018.xls")

comparative <- comparative %>% 
  mutate(policys= scale(comparative$policy)) %>%
  mutate(politicals = scale(comparative$political)) %>% 
  select(Quarter, policys, politicals)

ggplot(data = comparative,aes(x=Quarter, y = policys))+
  geom_line()


ggplot(data = comparative,aes(x=Quarter, y = politicals))+
  geom_line()

pol_quarter <- pol_refine_pe %>% 
  select(year_month, composite) %>% 
  mutate(quarter = as.yearqtr(year_month, format="%Y-%m")) %>% 
  group_by(quarter) %>% 
  mutate(index = mean(composite)) %>% 
  select(quarter, index) %>% 
  unique()

comparative <- comparative %>% 
  mutate(quarter = as.yearqtr(Quarter)) %>% 
  select(quarter, politicals)

setDT(comparative)
setDT(pol_quarter)

comparative_political <- comparative[pol_quarter, on = c('quarter')]
comparative_political <- na.omit(comparative_political) 

colnames(comparative_political) <- c("quarter","comparative", "political_refined")

ggplot(data = comparative_political, aes(x = quarter))+
  geom_line(aes(y = political_refined, color = "political_refined"))+
  geom_line(aes(y= comparative, color="comparative"))+
  theme_base()+
  labs()

fis_quarter <- fis_refine_pe %>% 
  select(year_month) %>% 
  mutate(fiscal= fis_refine_pe$composite) %>% 
  mutate(quarter = as.yearqtr(year_month, format="%Y-%m")) %>% 
  group_by(quarter) %>% 
  mutate(fiscal_qtr = mean(fiscal)) %>% 
  select(quarter, fiscal_qtr) %>% 
  unique()

mon_quarter <- mon_refine_pe %>% 
  select(year_month) %>% 
  mutate(monetary= mon_refine_pe$composite) %>% 
  mutate(quarter = as.yearqtr(year_month, format="%Y-%m")) %>% 
  group_by(quarter) %>% 
  mutate(monetary_qtr = mean(monetary)) %>% 
  select(quarter, monetary_qtr) %>% 
  unique()

policies <- fis_quarter 

setDT(policies)
setDT(mon_quarter)

policies <- policies[mon_quarter, on = c('quarter')]


comparative <- comparative %>% 
  mutate(quarter = as.yearqtr(Quarter)) %>% 
  select(quarter, policys)

 
setDT(comparative)
setDT(policies)

comparative_policies<- comparative[policies, on = c('quarter')]
comparative_policies <- na.omit(comparative_policies) 

colnames(comparative_policies) <- c("quarter","comparative", "fiscal", "monetary")

ggplot(data = comparative_policies, aes(x = quarter))+
  geom_line(aes(y = fiscal, color = "fiscal"))+
  geom_line(aes(y= monetary, color = "monetary"))+
  geom_line(aes(y= comparative, color="comparative"))+
  theme_base()
