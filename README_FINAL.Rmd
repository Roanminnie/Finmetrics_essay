...
library(rmsfuns)
load_pkg(c("tidyverse","SentimentAnalysis","tbl2xts","ggthemes","ggplot2", "furrr", "viridisLite", "viridis", "lubridate", "zoo", "strucchange"))
```
...
```{r}
year.range.major <- function(political_naive, column = "year_month", n = 4){
  range <- diff(range(political_naive[,column]))
  range.num <- as.numeric(range)
  major = max(pretty((range.num/12)/n))
...
stat.desc(pol_refine_pe$composite)
